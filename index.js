...
for (var j = 1; j < finances.length; j++) {
    diff.push([finances[j][0],finances[j][1] - finances[j - 1][1]])
}
...
var average = change / (month - 1);
const result = average.toFixed(2);
const resultaverage = parseFloat(result);
...
for (var m = 0; m < diff.length; m++) {
    if (diff[m][1] > increase) {
        increase = diff[m][1];
        monthincrease = diff[m][0];
    }
}
...
for (var n = 0; n < diff.length; n++) {
    if (diff[n][1] < decrease) {
        decrease = diff[n][1];
        monthdecrease = diff[n][0]; 
    }
}
console.log(decrease);
console.log(monthdecrease);

//Finance Analysis Output

console.log("Financial Analysis\n-------------------");
console.log(`Total Months: ${month}`);
console.log(`Total: $${sum}`)
console.log(`Average Change: $${resultaverage}`)
